...
2. 正常写一个场景，不要在每轮对话后整理全部状态。
3. 场景结束时点击“生成场景收束草稿”（Prepare scene close）。
4. 审核记忆与 `storyState`，尤其检查正史、作者计划和人物关系。
5. 点击“保存已审核草稿”（Save reviewed draft）。
6. 到达重要分歧点时创建 checkpoint；若要在分支保存不同状态，先复制并
   重新绑定聊天世界书。
...
Agent 不得在共享世界书未隔离时保存互相冲突的主线和分支 Story State。
