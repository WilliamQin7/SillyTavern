...
1. **可选**：在“小说项目”页创建或选择轻量项目，列出可复用的人物、World
   Info 和剧情快照；已有项目默认值时，可用“一键准备当前聊天”完成绑定、
   当前章/场景选择和写作上下文启用。准备区就在项目选择器下方；只有状态明确
   显示“模型上下文已验证”，且生成容量足以覆盖本场预计字数，才算真的可写。
2. 在“角色”页生成或迁移 Character Card，校验后导入。
3. **可选**：已有章纲时，在“规划”页整理并保存 Story Plan，再只启用当前
   章或场景；没有 Story Plan 时，以下流程完全不变。
4. **可选**：在“写作控制”页审核并保存 Writing Profile，预览当前场景实际
   会看到的上下文，再确认启用。
5. 回到“小说项目”查看准备收据；容量不足时确认“应用推荐写作容量”，然后点击
   “把当前场景请求放入输入框”，审核后使用正常发送按钮。该按钮不会自动发送。
6. 正常写一个场景，不要在每轮对话后整理全部状态。
7. 场景结束时点击“生成场景收束草稿”（Prepare scene close）。
8. 审核记忆、`storyState` 与 `narrativeLedgerDelta`，尤其检查正史、作者
   计划、人物关系和重复痕迹。
9. 点击“保存已审核草稿”（Save reviewed draft）。
10. 到达重要分歧点时创建 checkpoint；若要在分支保存不同状态，先复制并
   重新绑定聊天世界书。
...
1. 打开 **Extensions → Amy Creator Studio → 小说项目**。
...
若项目已经保存剧情快照和默认 Writing Profile，可改点“一键准备当前聊天”。
...
项目页顶部的准备区会直接显示当前章/场景、Writing Profile 和模型上下文校验
结果。需要换焦点时点击“调整章节 / 场景焦点”，系统会跳到“规划”页顶部；选择
...
本场目标字数可直接在准备区调整，不必再到其他页寻找；调整后需重新“一键准备”
以刷新包含该目标的活动写作上下文。
...
“把当前场景请求放入输入框”只在项目、当前焦点、Writing Profile/计划验证和
容量检查全部通过后可用。它会生成包含当前章/场、目标字数和防止重复索要焦点
的正文请求，先展示确认，再替换输入框内容；不会自动发送、写入正史或接受生成
结果。项目中的人物和 World Info 数量仅在收据中标为引用，不能宣称它们本轮
全部被强制注入。
...
- 仅“绑定到当前聊天”不会自动控制剧情或写法；“载入为待审核草稿”仍保留逐步
  审核流程。用户明确确认“一键准备当前聊天”时，才会把项目内已经审核保存的
  剧情快照和默认 Writing Profile 一次性复制到当前聊天并启用。
