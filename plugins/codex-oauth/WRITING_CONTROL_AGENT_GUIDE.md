...
0. 当前聊天是否绑定 Story Project；若项目已保存剧情快照和默认 Profile，且
   用户同意一次持久化确认，优先点击“一键准备当前聊天”。按钮完成绑定、载入、
...
用户审核保存的前提下，使用“一键准备当前聊天”，接受一次汇总确认即可。
...
本场目标字数可在项目准备区直接修改；修改会使旧编译上下文过期，Agent 必须
再次执行“一键准备当前聊天”，不能沿用修改前的验证收据。

需要调整焦点时，点击项目页准备区的“调整章节 / 场景焦点”；它应跳到“规划”页
...
全部就绪后，可点击“把当前场景请求放入输入框”。Agent 必须审核确认框中的请求
